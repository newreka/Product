...
UNSET_PRODUCT_ID = '12aidmsbsp2e29'
...
if PROMPT_FOR_PRODUCTID_AND_SN is True or productid == UNSET_PRODUCT_ID:
    print("The default Product Identity is: {}".format(productid))
    productidok = input("If OK, hit return, if you prefer a different Identity, type it here: ")
    if productidok != "":
...
while LOOP:
    ...
    # Read data from Exosite and show the JSON as received
    status, resp = READ('owner')
    ...
    if not status and resp == 304:
        ...
        pass
    if status:
        print "JSON read from Dashboard: " ,(resp)
